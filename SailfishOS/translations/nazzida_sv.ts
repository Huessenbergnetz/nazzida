<?xml version="1.0" ?><!DOCTYPE TS><TS version="2.1" language="sv" sourcelanguage="en">
<context>
    <name/>
    <message id="nazzida-author-role">
        <source>Main developer, Nazzida creator</source>
        <oldsource>Main developer, Fuoten creator</oldsource>
        <extracomment>contributor role</extracomment>
        <translation>Primär utvecklare, Nazzida-skapare</translation>
    </message>
    <message id="nazzida-author-section">
        <source>Author</source>
        <extracomment>the section a contributor belongs to</extracomment>
        <translation>Utvecklare</translation>
    </message>
    <message id="naz-title-about">
        <source>About</source>
        <extracomment>Page title and pull down menu entry</extracomment>
        <translation>Om</translation>
    </message>
    <message id="naz-about-app-desc">
        <source>Nazzida is the old high german word for liquid. This applicaton helps you to manage a personal fluid balance protocol if you need to log your daily liquid input and output.</source>
        <translation>Nazzida är det gamla högtyska ordet för vätska. Denna app hjälper dig att hantera ett personligt vätskebalansprotokoll om du behöver logga ditt dagliga vätskeintag och -utsöndring.</translation>
    </message>
    <message id="naz-germany">
        <source>Germany</source>
        <translation>Tyskland</translation>
    </message>
    <message id="naz-title-add-person">
        <source>Add Person</source>
        <extracomment>Page title and pull down menu enty</extracomment>
        <translation>Lägg till person</translation>
    </message>
    <message id="naz-textfield-firstname">
        <source>First name</source>
        <extracomment>text field label</extracomment>
        <translation>Förnamn</translation>
    </message>
    <message id="naz-textfield-lastname">
        <source>Last name</source>
        <extracomment>text field label</extracomment>
        <translation>Efternamn</translation>
    </message>
    <message id="naz-textfield-size">
        <source>Size in cm</source>
        <extracomment>text field label</extracomment>
        <translation>Längd i cm</translation>
    </message>
    <message id="naz-textfield-birthday">
        <source>Birthday</source>
        <extracomment>text field label</extracomment>
        <translation>Födelsedag</translation>
    </message>
    <message id="naz-textfield-daystarts">
        <source>Day start time</source>
        <extracomment>text field label</extracomment>
        <translation>Dagen börjar</translation>
    </message>
    <message id="naz-textfield-daystarts-desc">
        <source>The day start time is used to determine when a new day starts. Liquid intake or output that occurs before this time will be assigned to the previous day. The first toilet after getting up is usually still assigned to the balance of the previous day. So you should choose a time when you nominally get up.</source>
        <oldsource>The day start time is used to determine when a new day starts.</oldsource>
        <extracomment>text field description</extracomment>
        <translation>Dagen börjar, används för att bestämma när en ny dag börjar. Vätskeintag eller -utsöndring som sker före denna tidpunkt kommer att hänföras till föregående dag. Det första toalettbesöket, efter att man gått upp, tilldelas vanligtvis fortfarande balansen för föregående dag. Du bör alltså välja en tidpunkt då du nominellt går upp.</translation>
    </message>
    <message id="naz-first-page-header-desc">
        <source>List of logged people</source>
        <extracomment>header page description for first page</extracomment>
        <translation>Lista över loggade personer</translation>
    </message>
    <message id="naz-full-name">
        <source>%1 %2</source>
        <extracomment>%1 will be the first, %2 the last name</extracomment>
        <translation>%1 %2</translation>
    </message>
    <message id="naz-err-failed-open-db">
        <source>Failed to open database %1: %2</source>
        <extracomment>error message, %1 will be the path to the database file, %2 will be the database error message</extracomment>
        <translation>Kunde inte öppna databas %1: %2</translation>
    </message>
    <message id="naz-err-failed-execute-db-query">
        <source>Failed to execute database query: %1</source>
        <extracomment>error message, %1 will be replaced by the database error message</extracomment>
        <translation>Kunde inte utföra databasförfrågan: %1</translation>
    </message>
    <message id="naz-err-failed-prepare-db-query">
        <source>Failed to prepare database query: %1</source>
        <extracomment>error message, %1 will be replaced by the database error message</extracomment>
        <translation>Kunde inte förbereda databasförfrågan: %1</translation>
    </message>
    <message id="naz-err-failed-create-data-dir">
        <source>Failed to create the data directory %1</source>
        <extracomment>error message, %1 will be the directory path</extracomment>
        <translation>Kunde inte skapa datamappen: %1</translation>
    </message>
    <message id="naz-err-failed-db-migrations">
        <source>Failed to perform database migrations: %1</source>
        <extracomment>error message, %1 will be replaced by the migration error</extracomment>
        <translation>Kunde inte utföra databasmigreringar: %1</translation>
    </message>
    <message id="naz-remorse-delete">
        <source>Delete %1 %2</source>
        <extracomment>remorse item text, %1 will be the first, %2 the second name</extracomment>
        <translation>Ta bort %1 %2</translation>
    </message>
    <message id="naz-combo-label-sex">
        <source>Sex</source>
        <extracomment>ComboBox label</extracomment>
        <translation>Kön</translation>
    </message>
    <message id="naz-combo-sex-item-male">
        <source>Male</source>
        <extracomment>ComboBox entry</extracomment>
        <translation>Man</translation>
    </message>
    <message id="naz-combo-sex-item-female">
        <source>Female</source>
        <extracomment>ComboBox entry</extracomment>
        <translation>Kvinna</translation>
    </message>
    <message id="naz-combo-sex-item-other">
        <source>Other</source>
        <extracomment>ComboBox entry</extracomment>
        <translation>Annat</translation>
    </message>
    <message id="naz-title-edit-person">
        <source>Edit %1 %2</source>
        <extracomment>dialog header title, %1 will be replaced by the first name, %2 by the last name</extracomment>
        <translation>Redigera %1 %2</translation>
    </message>
    <message id="naz-size-in-cm">
        <source>%1 cm</source>
        <extracomment>%1 will be the body size in cm</extracomment>
        <translation>%1 cm</translation>
    </message>
    <message id="naz-age-in-years" numerus="yes">
        <source>%n year(s)</source>
        <oldsource>%1 years</oldsource>
        <extracomment>%1 will be the age in years</extracomment>
        <translation><numerusform>%n år</numerusform><numerusform>%n år</numerusform></translation>
    </message>
    <message id="naz-title-settings">
        <source>Settings</source>
        <extracomment>Page title and pull down menu entry</extracomment>
        <translation>Inställningar</translation>
    </message>
    <message id="naz-settings-section-appearance">
        <source>Appearance</source>
        <extracomment>settings section header</extracomment>
        <translation>Utseende</translation>
    </message>
    <message id="naz-settings-language-label">
        <source>Language</source>
        <extracomment>ComboBox label on the settings page</extracomment>
        <translation>Språk</translation>
    </message>
    <message id="naz-settings-language-desc">
        <source>To change the language, you have to restart the application.</source>
        <extracomment>Description for a combo box on the settings page to select the application language</extracomment>
        <translation>Du måste starta om appen för att byta språk.</translation>
    </message>
    <message id="naz-fluid-balance-protocol">
        <source>Fluid Balance Protocol</source>
        <oldsource>Fluid Balance Protol</oldsource>
        <extracomment>label on a list of different entries</extracomment>
        <translation>Protokoll för vätskebalans</translation>
    </message>
    <message id="naz-err-failed-foreign-keys-pragma">
        <source>Failed to enable foreign keys pragma: %1</source>
        <extracomment>error message, %1 will be the database error message</extracomment>
        <translation>Det gick inte att aktivera främmande nycklar: %1</translation>
    </message>
    <message id="naz-add-input-output">
        <source>Add In‐ or Output</source>
        <extracomment>Dialog title and pull down menu entry, means liquids</extracomment>
        <translation>Tillägg eller utdata</translation>
    </message>
    <message id="naz-combo-label-inouttype">
        <source>In‐ or Output</source>
        <extracomment>label for a combobox on th add liquid dialog</extracomment>
        <translation>In- eller utdata</translation>
    </message>
    <message id="naz-combo-inouttype-item-in">
        <source>Drinks/Soups</source>
        <extracomment>ComboBox item</extracomment>
        <translation>Drycker/soppor</translation>
    </message>
    <message id="naz-combo-inouttype-item-iniv">
        <source>Intravenous</source>
        <extracomment>ComboBox item</extracomment>
        <translation>Intravenöst</translation>
    </message>
    <message id="naz-combo-inouttype-item-out">
        <source>Excretion</source>
        <extracomment>ComboBox item, excretion of body fluids</extracomment>
        <translation>Utsöndring</translation>
    </message>
    <message id="naz-full-liquid-title">
        <source>%1 ml %2</source>
        <extracomment>%1 will be the amount of the liquid, %2 will be the liquid name</extracomment>
        <translation>%1 ml %2</translation>
    </message>
    <message id="naz-textfield-liquid-amount-label">
        <source>Amount in ml</source>
        <extracomment>text field label</extracomment>
        <translation>Mängd i ml</translation>
    </message>
    <message id="naz-textfield-liquid-name-label">
        <source>Name</source>
        <extracomment>text field label</extracomment>
        <translation>Namn</translation>
    </message>
    <message id="naz-remorse-delete-liquid">
        <source>Delete %1 ml %2 from %3</source>
        <extracomment>remorse item text, %1 will be the liquid amout, %2 the liquid name, %3 will be the datetime the liquid has been consumed or excreted</extracomment>
        <translation>Ta bort %1 ml %2 från %3</translation>
    </message>
    <message id="naz-edit-input-output">
        <source>Edit In- or Output</source>
        <extracomment>dialog title</extracomment>
        <translation>Redigera in- eller utdata</translation>
    </message>
    <message id="naz-name-day-total">
        <source>%1 %2, %3, %4 ml</source>
        <extracomment>%1 will be the first name, %2 the second name, %3 will be the date in short format, %4 will be the liquid amount in ml</extracomment>
        <translation>%1 %2, %3, %4 ml</translation>
    </message>
    <message id="naz-textfield-transpire">
        <source>Transpire in ml</source>
        <extracomment>text field label</extracomment>
        <translation>Transpiration i ml</translation>
    </message>
    <message id="naz-textfield-transpire-desc">
        <source>Approximate daily transpiration. The body of an adult loses about 500 ml daily through perspiration. This is used when calculating the difference between fluid intake and fluid output.</source>
        <extracomment>text field description</extracomment>
        <translation>Ungefärlig daglig transpiration. En vuxens kropp förlorar ca 500 ml dagligen genom svettning. Detta används vid beräkning av skillnaden mellan vätskeintag och -utsöndring.</translation>
    </message>
    <message id="naz-privacy-0">
        <source>What information does Nazzida collect?</source>
        <translation>Vilken information samlar Nazzida in?</translation>
    </message>
    <message id="naz-privacy-1">
        <source>Nazzida helps you to manage a personal fluid balance protocol if you need to log your daily liquid input and output. To do this, it stores your input into a local SQLite database on your device. It saves only data that you enter yourself in the application.</source>
        <translation>Nazzida hjälper dig att hantera ett personligt vätskebalansprotokoll om du behöver logga ditt dagliga vätskeintag och avgång. För att göra detta lagrar den dina indata i en lokal SQLite-databas på din enhet. Det sparar bara data som du anger själv i programmet.</translation>
    </message>
    <message id="naz-privacy-2">
        <source>What does Nazzida use your data for?</source>
        <translation>Vad använder Nazzida dina data till?</translation>
    </message>
    <message id="naz-privacy-3">
        <source>Nazzida uses your stored data to show you your daily liquid balance data as well as every data entry you provided to the application. The data is only stored on your local device and is not shared to other parties.</source>
        <translation>Nazzida använder dina lagrade data för att visa dig din dagliga vätskebalansdata samt varje datainmatning du anger i appen. Uppgifterna lagras endast på din lokala enhet och delas inte med andra parter.</translation>
    </message>
    <message id="naz-privacy-4">
        <source>Does Nazzida disclose any information to outside parties?</source>
        <translation>Lämnar Nazzida ut någon information till utomstående parter?</translation>
    </message>
    <message id="naz-privacy-5">
        <source>No, Nazzida only stores the data local on your device. Nazzida does not send this data to the author of Nazzida or any other third parties.</source>
        <translation>Nej, Nazzida lagrar bara data lokalt på din enhet. Nazzida skickar inte dessa uppgifter till Nazzidas skapare eller någon annan tredje part.</translation>
    </message>
    <message id="nazh-privacy-6">
        <source>Your consent</source>
        <translation>Ditt samtycke</translation>
    </message>
    <message id="nazh-privacy-7">
        <source>By using Nazzida, you consent to this privacy policy.</source>
        <oldsource>By using Nazhzida, you consent to this privacy policy.</oldsource>
        <translation>Genom att använda Nazzida samtycker du till denna integritetspolicy.</translation>
    </message>
    <message id="nazh-privacy-8">
        <source>Contact</source>
        <translation>Kontakt</translation>
    </message>
    <message id="naz-privazy-policy">
        <source>Privacy policy</source>
        <extracomment>Label and page header</extracomment>
        <translation>Integritetspolicy</translation>
    </message>
    <message id="naz-people-list-empty-text">
        <source>No people added</source>
        <extracomment>View placeholder text</extracomment>
        <translation>Inga personer har lagts till</translation>
    </message>
    <message id="naz-people-list-empty-hint">
        <source>Pull down to add people</source>
        <extracomment>View placeholder hint</extracomment>
        <translation>Dra neråt för att lägga till personer</translation>
    </message>
    <message id="naz-liquids-list-empty-text">
        <source>No liquids added</source>
        <extracomment>View placeholder text</extracomment>
        <translation>Ingen vätska tillagd</translation>
    </message>
    <message id="naz-liquids-list-empty-hint">
        <source>Pull down to add liquids</source>
        <extracomment>View placeholder hint</extracomment>
        <translation>Dra neråt för att lägga till vätska</translation>
    </message>
    <message id="nazzida-contributor-eson-role">
        <source>Swedish translation</source>
        <extracomment>contributor role</extracomment>
        <translation>Svensk översättning</translation>
    </message>
    <message id="nazzida-thanks-to-section">
        <source>Thanks to</source>
        <extracomment>the section a contributor belongs to</extracomment>
        <translation>Tack till</translation>
    </message>
    <message id="naz-textarea-note-label">
        <source>Note (optional)</source>
        <extracomment>text area label</extracomment>
        <translation>Anteckning (valfritt)</translation>
    </message>
    <message id="naz-add-weight">
        <source>Add weight</source>
        <extracomment>Dialog title and pull down menu entry</extracomment>
        <translation>Lägg till vikt</translation>
    </message>
    <message id="naz-textfield-weight-label">
        <source>Weight in %1</source>
        <extracomment>text field label, %1 will be replaced by the unit, like kg</extracomment>
        <translation>Vikt i %1</translation>
    </message>
    <message id="naz-weight-protocol">
        <source>Weight Protocol</source>
        <extracomment>label on a list of different entries</extracomment>
        <translation>Viktprotokoll</translation>
    </message>
    <message id="naz-weight-with-unit">
        <source>%1 %2</source>
        <extracomment>weight with unit, %1 will be the weight like 70.3 and %2 will be the unit like kg</extracomment>
        <translation>%1 %2</translation>
    </message>
    <message id="naz-remorse-delete-weight">
        <source>Delete %1 from %2</source>
        <oldsource>Delete %1 from %3</oldsource>
        <extracomment>remorse item text, %1 will be the weight amount like 70.5 kg, %2 will be the datetime the weight has been added</extracomment>
        <translation>Ta bort %1 från %2</translation>
    </message>
    <message id="naz-weights-list-empty-text">
        <source>No weights added</source>
        <extracomment>View placeholder text</extracomment>
        <translation>Ingen vikt tillagd</translation>
    </message>
    <message id="naz-weights-list-empty-hint">
        <source>Pull down to add weights</source>
        <extracomment>View placeholder hint</extracomment>
        <translation>Dra neråt för att lägga till vikt</translation>
    </message>
    <message id="naz-textfield-date">
        <source>Date</source>
        <extracomment>text field label</extracomment>
        <translation>Datum</translation>
    </message>
    <message id="naz-textfield-time">
        <source>Time</source>
        <extracomment>text field label</extracomment>
        <translation>Tid</translation>
    </message>
    <message id="naz-contextmenu-edit">
        <source>Edit</source>
        <extracomment>context menu item text</extracomment>
        <translation>Redigera</translation>
    </message>
    <message id="naz-contextmenu-del">
        <source>Delete</source>
        <extracomment>context menu item text</extracomment>
        <translation>Ta bort</translation>
    </message>
    <message id="naz-dialog-accept-add">
        <source>Add</source>
        <extracomment>Accept text for add dialogs</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-dialog-accept-edit">
        <source>Apply</source>
        <extracomment>Accept text on edit dialog headers</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-edit-weight">
        <source>Edit Weight</source>
        <extracomment>dialog title</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-weight-bmi">
        <source>BMI: %1 (%2)</source>
        <oldsource>BMI: %1</oldsource>
        <extracomment>%1 will be replaced by the body mass index like 22.1, %2 will be replaced by a BMI classification like Normal weight or Pre-obese</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-general-underweight">
        <source>Underweight</source>
        <extracomment>BMI general classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-general-normal">
        <source>Normal weight</source>
        <extracomment>BMI general classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-general-overweight">
        <source>Overweight</source>
        <extracomment>BMI general classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-general-obese">
        <source>Obese</source>
        <extracomment>BMI general classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-severe-thinness">
        <source>Severe thinness</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-moderate-thinness">
        <source>Moderate thinness</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-mild-thinness">
        <source>Mild thinness</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-normal">
        <source>Normal weight</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-pre-obese">
        <source>Pre-obese</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-obese-1">
        <source>Obese class I</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-obese-2">
        <source>Obese class II</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
    <message id="naz-bmi-detailed-obese-3">
        <source>Obese class III</source>
        <extracomment>BMI deatiled classification</extracomment>
        <translation type="unfinished"/>
    </message>
</context>
</TS>